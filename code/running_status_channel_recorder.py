...
def main():
    ...
    if len(sys.argv) != 2:
        time_print(f"Script exit: sys.argv has not received correct arguments to select channel: {sys.argv}")
    ...
    while active:
        ...
        for key, val in running.items():
            if key not in event_list:
                ...
                indent_print(f"{prog_info[1]} to {prog_info[2]} - duration {prog_info[0]}")
...
def record_stream(instream, outfile):
    ...
    inst = vlc.Instance('--demux=dump', f"--demuxdump-file={outfile}", "--demuxdump-append")
